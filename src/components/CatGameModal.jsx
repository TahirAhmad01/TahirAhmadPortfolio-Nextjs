...
import { Trophy, RefreshCw, X, Sparkles, Award, Zap } from "lucide-react";

const BUG_TYPES = [
  { icon: "🐞", name: "Ladybug", points: 10, speed: 1.5 },
  { icon: "🪲", name: "Beetle", points: 15, speed: 2.2 },
  { icon: "🦟", name: "Mosquito", points: 25, speed: 3.2 },
  { icon: "🐛", name: "Caterpillar", points: 20, speed: 1.0 },
];
...
export default function CatGameModal({ isOpen, onClose }) {
  ...
  useEffect(() => {
    ...
    const moveInterval = setInterval(() => {
      setBugs((prevBugs) =>
        prevBugs.map((bug) => {
          let newX = bug.x + Math.cos(bug.angle) * bug.speed * 4;
          let newY = bug.y + Math.sin(bug.angle) * bug.speed * 4;
          ...
          return {
            ...
            angle: newAngle + (Math.random() - 0.5) * 0.2,
          };
        })
      );
    }, 50);
  ...
  return (
    <div className="fixed inset-0 z-[100000] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-fade-in select-none">
      <div className="relative w-full max-w-lg bg-slate-900 border border-purple-500/40 rounded-3xl shadow-2xl overflow-hidden flex flex-col font-sans">
        {/* Header */}
        <div className="flex justify-between items-center px-6 py-4 bg-slate-800/80 border-b border-purple-500/20">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-purple-300/80">Swat bugs with cat reflexes!</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        >
          ...
          {gameState === "idle" && (
            <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-6 bg-slate-950/90 z-20">
              ...
              <h4 className="text-xl font-bold text-white mb-2">Ready to Catch Bugs?</h4>
              <p className="text-xs text-gray-300 max-w-xs mb-6">
                Tap or click as many crawling bugs as you can in 30 seconds! Build up combos for bonus points.
              </p>
              <button
                onClick={startGame}
          ...
          {gameState === "playing" && (
            <>
              <div className="absolute top-3 left-3 right-3 flex justify-between items-center z-10 text-xs font-bold">
                <div className="bg-slate-800/80 text-amber-400 px-3 py-1.5 rounded-xl border border-amber-400/20 flex items-center gap-1.5 shadow-md">
                  <Sparkles size={14} />
                  <span>Score: {score}</span>
                </div>
                <div className="bg-slate-800/80 text-cyan-400 px-3 py-1.5 rounded-xl border border-cyan-400/20 shadow-md">
                  ⏱️ {timeLeft}s
                </div>
                <div className="bg-slate-800/80 text-purple-300 px-3 py-1.5 rounded-xl border border-purple-400/20 flex items-center gap-1 shadow-md">
                  <Trophy size={14} />
              ...
              {splatters.map((s) => (
                <div
                  key={s.id}
                  className="absolute text-amber-400 font-black text-sm pointer-events-none animate-bounce z-20"
